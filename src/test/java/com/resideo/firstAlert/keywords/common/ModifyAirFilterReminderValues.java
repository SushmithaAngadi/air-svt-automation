...
public class ModifyAirFilterReminderValues extends Keyword {
	...
	public ModifyAirFilterReminderValues(TestCases testCase, TestCaseInputs inputs, ArrayList<String> exampleData) {
	...
	// When user modifies "Air Filter Reminder 1" reminder "30" "Run Time Days" in FA App

	@When(value = "^user modifies (.+) reminder (.+) (.+) in FA App$")
	@Override
	@KeywordStep(gherkins = "^user modifies (.+) reminder (.+) (.+) in FA App$")
	public boolean keywordSteps() {
		...
		try {
			if (testCase.isTestSuccessful()) {
				flag = FA_AlertUtils.modifyAirFilterReminder(testCase, filterName, filterValue, timeUnit);
			} else {
				flag = false;
				Keyword.ReportStep_Fail_WithOut_ScreenShot(testCase, FailType.FUNCTIONAL_FAILURE,
						"Scenario steps failed already, hence skipping the verification");
