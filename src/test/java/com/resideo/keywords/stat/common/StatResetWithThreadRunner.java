 ...
 * </p>
 * <b>Author : Hariharan Murugan (247185)</b></br>
 * 
 * Pre-condition :NA </br>
 * Keyword-steps : </br>
 ...
 */
public class StatResetWithThreadRunner extends Keyword {

	private TestCases testCase;
	public boolean flag = true;
	ArrayList<String> keywordData;
	String commandToStat;
	boolean isThreadToPlay;
	boolean isThreadToContinue;
	KeepAppActive appActive;

	public StatResetWithThreadRunner(TestCases testCase, TestCaseInputs inputs, ArrayList<String> data) {
	...
	public boolean preCondition() throws KeywordException {
		...
		commandToStat = keywordData.get(0).toUpperCase();
		isThreadToPlay = Boolean.valueOf(keywordData.get(1).toLowerCase());
		isThreadToContinue = Boolean.valueOf(keywordData.get(2).toLowerCase());
	...
	@And(value = "^set stat to (.+) with thread state is (.+) and thread need to continue (.+)$")
	@Override
	@KeywordStep(gherkins = "^set stat to (.+) with thread state is (.+) and thread need to continue (.+)$")
		...
		try {
			if (keywordData.get(0).toUpperCase().equalsIgnoreCase("DEFAULT NVM")) {
				...
				flag = StatCommands.getInstance().cattCommandsStub
						.unCheckReset(
								Stat_Reset.newBuilder().setResetMode(ResetMode.AAT_SYSTEM_RESET_DEFAULT_NVM).build())
						.getBoolVal();

			} else if (keywordData.get(0).toUpperCase().equalsIgnoreCase("FACTORY DEFAULT")) {
				flag = StatCommands.getInstance().cattCommandsStub
						.unCheckReset(Stat_Reset.newBuilder().setResetMode(ResetMode.AAT_FACTORY_RESET).build())
						.getBoolVal();
