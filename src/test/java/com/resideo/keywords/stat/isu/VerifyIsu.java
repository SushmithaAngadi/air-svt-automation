...
public class VerifyIsu extends Keyword {
	...
	public VerifyIsu(TestCases testCase, TestCaseInputs inputs, ArrayList<String> exampleData) {
	...
	@Given(value = "^user verifies isu (.+) with value (.+)$")
	@Override
	@KeywordStep(gherkins = "^user verifies isu (.+) with value (.+)$")
		...
		if (testCase.isTestSuccessful()) {
			...
			if (exampleData.get(0) != "" || exampleData.get(0) != " " || exampleData.get(1) != ""
				...
				if(deviceName.contains("JASPER_SENIOR")) {
					flag = JasperUtils.verifyISU(testCase, isuType, isuVal);
				}else if(deviceName.contains("FLYCATCHER")) {
					flag = FlycatcherUtils.verifyISU(testCase, isuType, isuVal);
				}else if(deviceName.contains("THOR")) {
					flag = ThorUtils.verifyISU(testCase, isuType, isuVal);
				}
			} else {
				flag = false;
