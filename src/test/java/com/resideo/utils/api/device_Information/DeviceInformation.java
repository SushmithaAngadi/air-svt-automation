package com.resideo.utils.api.device_Information;
...
public class DeviceInformation {
	...
	public DeviceInformation(TestCases testCase, TestCaseInputs inputs, String macID) {
		this.testCase = testCase;
		deviceInformation = LyricUtils.getDeviceInformation(testCase, inputs, macID);
	}
	
	public DeviceInformation(TestCases testCase, TestCaseInputs inputs,long locationID, String macID) {
		this.testCase = testCase;
		deviceInformation = LyricUtils.getDeviceInformation(testCase, inputs,locationID, macID);
	}
	...
	public String getVacationCoolSetPoint() {
		...
		try {
			System.out.println(deviceInformation.getJSONObject("vacationHold").get("coolSetpoint"));
	...
	public JSONObject SyncDeviceInformation(TestCases testCase, TestCaseInputs inputs) {
		return deviceInformation = LyricUtils.getDeviceInformation(testCase, inputs);
	}
	...
	public String getDehumidifierValue() throws Exception {
		String HumidifierValue = "";
		if (deviceInformation != null) {
			HumidifierValue = deviceInformation.getJSONObject("settings").getJSONObject("dehumidifierSettings")
					.getJSONObject("changeableValues").getString("mode");
		} else {
			throw new Exception("Device Information not found");
		}
	 ...
	 * HashMap<String, String> setPoints = new HashMap<String, String>(); if
	 * (deviceInformation != null) { List<String> allowedModes = getAllowedModes();
	 * if (allowedModes.contains("Cool") && allowedModes.contains("Heat")) { String
	 * temp = String.valueOf(deviceInformation.getJSONObject("thermostat").get(
	 * "maxHeatSetpoint")); if (getThermostatUnits().equals("Celsius")) { temp =
	 * JasperSchedulingUtils.roundOffCelsiusData(testCase,
	 * JasperSchedulingUtils.convertFromFahrenhietToCelsius(testCase, temp)); }
	 * setPoints.put("MaxHeat", temp); temp =
	 * String.valueOf(deviceInformation.getJSONObject("thermostat").get(
	 * "maxCoolSetpoint")); if (getThermostatUnits().equals("Celsius")) { temp =
	 * JasperSchedulingUtils.roundOffCelsiusData(testCase,
	 * JasperSchedulingUtils.convertFromFahrenhietToCelsius(testCase, temp)); }
	 * setPoints.put("MaxCool", temp); temp =
	 * String.valueOf(deviceInformation.getJSONObject("thermostat").get(
	 * "minHeatSetpoint")); if (getThermostatUnits().equals("Celsius")) { temp =
	 * JasperSchedulingUtils.roundOffCelsiusData(testCase,
	 * JasperSchedulingUtils.convertFromFahrenhietToCelsius(testCase, temp)); }
	 * setPoints.put("MinHeat", temp); temp =
	 * String.valueOf(deviceInformation.getJSONObject("thermostat").get(
	 * "minCoolSetpoint")); if (getThermostatUnits().equals("Celsius")) { temp =
	 * JasperSchedulingUtils.roundOffCelsiusData(testCase,
	 * JasperSchedulingUtils.convertFromFahrenhietToCelsius(testCase, temp)); }
	 * setPoints.put("MinCool", temp); } else if (allowedModes.contains("Cool") &&
	 * !allowedModes.contains("Heat")) { String temp =
	 * String.valueOf(deviceInformation.getJSONObject("thermostat").get(
	 * "maxCoolSetpoint")); if (getThermostatUnits().equals("Celsius")) { temp =
	 * JasperSchedulingUtils.roundOffCelsiusData(testCase,
	 * JasperSchedulingUtils.convertFromFahrenhietToCelsius(testCase, temp)); }
	 ...
	 * JasperSchedulingUtils.roundOffCelsiusData(testCase,
	 * JasperSchedulingUtils.convertFromFahrenhietToCelsius(testCase, temp)); }
	 * setPoints.put("MinHeat", temp); } } return setPoints; }
	 */
	...
	/*
	 * public String getCoolSetPoints() { String coolSetPoints = " "; try { if
	 * (deviceInformation != null) {
	 * 
	 * coolSetPoints = deviceInformation.getJSONObject("thermostat").getJSONObject(
	 * "changeableValues") .get("coolSetpoint").toString();
	 * 
	 * } else { Keyword.ReportStep_Fail(testCase, FailType.FUNCTIONAL_FAILURE,
	 * "Get Heat Set Points : Not Connected to CHAPI. Returning \"\" value"); return
	 * ""; } if (getThermostatUnits().equals("Celsius")) { coolSetPoints =
	 * JasperSchedulingUtils.convertFromFahrenhietToCelsius(testCase,
	 * coolSetPoints); } else if (getThermostatUnits().equals("Fahrenheit")) {
	 * Double temp = Double.parseDouble(coolSetPoints); coolSetPoints =
	 * String.valueOf(temp.intValue()); } } catch (Exception e) {
	 * Keyword.ReportStep_Fail_WithOut_ScreenShot(testCase,
	 * FailType.FUNCTIONAL_FAILURE,
	 * "Get Stat Information  : Unable to get Current System mode for Stat - " +
	 * statName + " at location - " + locationName + " : Error occured - " +
	 * e.getMessage()); } return coolSetPoints; }
	 * 
	 */

	/*
	 * public String getIndoorTemperature() { String indoorTemp = " "; try { if
	 * (deviceInformation != null) {
	 * 
	 * indoorTemp =
	 * deviceInformation.getJSONObject("thermostat").get("indoorTemperature").
	 * toString();
	 * 
	 * } else { Keyword.ReportStep_Fail(testCase, FailType.FUNCTIONAL_FAILURE,
	 * "Get Indoor Temperature : Not Connected to CHAPI. Returning \"\" value");
	 * return ""; } if (getThermostatUnits().equals("Celsius")) { indoorTemp =
	 * JasperSchedulingUtils.convertFromFahrenhietToCelsius(testCase, indoorTemp); }
	 * else if (getThermostatUnits().equals("Fahrenheit")) { Double temp =
	 * Double.parseDouble(indoorTemp); indoorTemp = String.valueOf(temp.intValue());
	 * } } catch (Exception e) {
	 * Keyword.ReportStep_Fail_WithOut_ScreenShot(testCase,
	 * FailType.FUNCTIONAL_FAILURE,
	 * "Get Stat Information  : Unable to get Current System mode for Stat - " +
	 * statName + " at location - " + locationName + " : Error occured - " +
	 * e.getMessage()); } return indoorTemp; }
	 */
	/*
	 * public String getCurrentSetPoints() { String currentSetPoints = " "; try { if
	 * (deviceInformation != null) {
	 * 
	 * if (getThermoStatMode().equals("Heat")) { currentSetPoints =
	 * deviceInformation.getJSONObject("thermostat").getJSONObject(
	 * "changeableValues") .get("heatSetpoint").toString(); } else if
	 * (getThermoStatMode().equals("Cool")) { currentSetPoints =
	 * deviceInformation.getJSONObject("thermostat").getJSONObject(
	 * "changeableValues") .get("coolSetpoint").toString(); } else if
	 * (getThermoStatMode().equals("Auto")) { if
	 * (getThermostatModeWhenAutoChangeOverActive().equals("Heat")) {
	 * currentSetPoints = deviceInformation.getJSONObject("thermostat")
	 * .getJSONObject("changeableValues").get("heatSetpoint").toString(); } else if
	 * (getThermostatModeWhenAutoChangeOverActive().equals("Cool")) {
	 * currentSetPoints = deviceInformation.getJSONObject("thermostat")
	 * .getJSONObject("changeableValues").get("coolSetpoint").toString(); } }
	 * 
	 * } else { Keyword.ReportStep_Fail(testCase, FailType.FUNCTIONAL_FAILURE,
	 * "Get Current Set Points : Not Connected to CHAPI. Returning \"\" value");
	 * return ""; } Keyword.ReportStep_Pass(testCase, currentSetPoints); if
	 * (deviceInformation.getJSONObject("thermostat").getString("units").equals(
	 * "Celsius")) { currentSetPoints =
	 * JasperSchedulingUtils.convertFromFahrenhietToCelsius(testCase,
	 * currentSetPoints); } else if
	 * (deviceInformation.getJSONObject("thermostat").getString("units").equals(
	 * "Fahrenheit")) { Double temp = Double.parseDouble(currentSetPoints);
	 * currentSetPoints = String.valueOf(temp.intValue()); } } catch (Exception e) {
	 * 
	 * Keyword.ReportStep_Fail_WithOut_ScreenShot(testCase,
	 * FailType.FUNCTIONAL_FAILURE,
	 * "Get Stat Information  : Unable to get Current System mode for Stat - " +
	 * statName + " at location - " + locationName + " : Error occured - " +
	 * e.getMessage()); } return currentSetPoints; }
	 * 
	 * public String getHeatSetPoints() { String heatSetPoints = " "; try {
	 * 
	 * if (deviceInformation != null) {
	 * 
	 * heatSetPoints = deviceInformation.getJSONObject("thermostat").getJSONObject(
	 * "changeableValues") .get("heatSetpoint").toString();
	 * 
	 * 
	 * } else { Keyword.ReportStep_Fail(testCase, FailType.FUNCTIONAL_FAILURE,
	 * "Get Heat Set Points : Not Connected to CHAPI. Returning \"\" value"); return
	 * ""; } if (getThermostatUnits().equals("Celsius")) { heatSetPoints =
	 * JasperSchedulingUtils.convertFromFahrenhietToCelsius(testCase,
	 * heatSetPoints); } else if (getThermostatUnits().equals("Fahrenheit")) {
	 * Double temp = Double.parseDouble(heatSetPoints); heatSetPoints =
	 * String.valueOf(temp.intValue()); } } catch (Exception e) {
	 * Keyword.ReportStep_Fail_WithOut_ScreenShot(testCase,
	 * FailType.FUNCTIONAL_FAILURE,
	 * "Get Stat Information  : Unable to get Current System mode for Stat - " +
	 * statName + " at location - " + locationName + " : Error occured - " +
	 * e.getMessage()); } return heatSetPoints; }
	 */
