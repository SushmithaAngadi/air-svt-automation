...
import com.resideo.utils.resideo_app.InputVariables;
...
//		System.out.println("Chil URL " + chilURL);
	}
...
//		disableSSLCertificateChecking();
	...
	public long getlocationIDByMacID(TestCases testCase, CHILUtil chUtil, String chilURL, String macID) {
		...
		for (int i = 0; i < size; i++) {
			...
			try {
				String url = chilURL + "api/v3/locations/" + locations.get(locations.keySet().toArray()[i]);
				...
				if (connection != null) {
					...
					if (jsonObj.has("devices")) {
						...
						if (elementFound) {
							locationID = Long.valueOf(locations.get(locations.keySet().toArray()[i]));
							Keyword.ReportStep_Pass(testCase, "device Found in this Location ID, ID is :" + locationID);
							break;
						}
					}
				...
				Keyword.ReportStep_Fail(testCase, 0,
						"API Failed, Exception  getlocationIDByMacID(TestCases testCase, CHILUtil chUtil, String chilURL, String macID) occured due to "
								+ e.getMessage());
				e.printStackTrace();
			}
		}
	...
	public boolean registerChilThermostat(long locationID, String deviceID, String deviceName, String deviceType)
			throws Exception {
		boolean flag = false;
		HttpURLConnection result;
		if (isConnected) {
			String url = " ";
			url = chilURL + "api/v2/locations/" + locationID + "/Devices/";
			String body = "{\r\n\r\n\"deviceType\": \"" + deviceType
					+ "\",\r\n\"deviceclass\":\"Thermostat\",\r\n\"name\": \"" + deviceName
					+ "\",\r\n\"RegistrationDeviceProperties\" : { \"Mac\":\"" + deviceID
					+ "\" },\r\n\"coordinates\": {\r\n\"latitude\": 1.1,\r\n\"longitude\": 2.1\r\n}\r\n}";

			try {
				result = doPostRequest(url, body);
				...
				if (result.getResponseCode() == HttpURLConnection.HTTP_CREATED
					...
					if (obj.toString().contains("result")) {
						if (obj.get("result").toString().contains("Success")) {
							url = chilURL + "api/v2/locations/" + locationID + "/devices/initialize";
							body = "{\r\n\"deviceType\" : \"" + deviceType + "\",\r\n\"deviceId\" : \"" + deviceID
									+ "\",\r\n\"action\" : \"Datasync\"\r\n}";
							Thread.sleep(4000);
							result = doPostRequest(url, body);
						...
						if (obj.get("message").toString().toUpperCase()
								.contains("Device is Already Registered with the user")) {
							flag = false;
						}
					}
				} else {
	...
	public void FeedDataIntoIOTHub(String deviceID, String payload, String xappUrl, String from, String method)
		...
		try {
			...
			client.sendEventAsync(message, (MessageSentCallback) new EventCallback(), message);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
	...
	public int setCoolSetPoint(long locationID, String deviceID, float changed_CoolSetpoint) {
		int result = -1;
		try {
			if (isConnected) {
				String url = chilURL
						+ String.format("api/locations/%s/devices/%s/thermostat/coolSetpoint", locationID, deviceID);
				String headerData = String.format("{\"thermostatSetpoint\":\"%s\"}", changed_CoolSetpoint);
				try {
					result = doPutRequest(url, headerData).getResponseCode();
				} catch (IOException e) {
					e.printStackTrace();
	...
	public int setHeatSetPoint(long locationID, String deviceID, float setpoint) {
		int result = -1;
		try {
			if (isConnected) {
				String url = chilURL
