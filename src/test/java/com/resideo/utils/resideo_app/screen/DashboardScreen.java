...
public class DashboardScreen extends MobileScreens {

	private static final String screenName = "Dashboard";

	public DashboardScreen(TestCases testCase) {
	...
	public String getDeviceTempVisibleOnDashboard() {
	...
	public boolean selectLocationFromDashBoard_NonStatic(TestCases testCase, String locationToBeSelected) {
		boolean flag = true;
	 ...
	 * 
	 * DeviceInformation statInfo = new DeviceInformation(testCase, inputs); String
	 * chilDeviceIndoorTemperature = statInfo.getIndoorTemperature();
	 * 
	 * if(testCase.getPlatform().toUpperCase().contains("IOS")==false) { String
	 * actualValue = MobileUtils.getFieldValue(objectDefinition, testCase,
	 * "ThermostatTemperature");
	 * if(actualValue.contains(chilDeviceIndoorTemperature) &&
	 * actualValue.toUpperCase().contains("INSIDE")) { return true; } else { return
	 * false; } } else { String
	 * thermostatname=inputs.getInputValue("LOCATION1_DEVICE1_NAME"); WebElement
	 * ele=MobileUtils.getMobElement(testCase,"xpath",
	 * "//XCUIElementTypeStaticText[@name='subTitle_"+thermostatname+"']"); String
	 ...
	 * CoachMarkUtils.closeCoachMarks(testCase); return true; } return false; }
	 */
	 ...
	 * String currentSetPoint=""; // boolean flag = true; int maxSetPointInt=0;
	 * float maxSetPointFloat=0; String maxSetPoint=""; boolean
	 * systemIsCelsius=false; DeviceInformation statInfo = new
	 * DeviceInformation(testCase, inputs); String
	 * currentSystemMode=statInfo.getThermoStatMode(); HashMap<String, String>
	 * setPoints = new HashMap<String, String>(); try {
	 * setPoints=statInfo.getDeviceMaxMinSetPoints(); } catch (Exception e) {
	 * e.printStackTrace(); } if(currentSystemMode.toUpperCase().contains("HEAT")) {
	 * maxSetPoint= setPoints.get("MaxHeat"); } else
	 * if(currentSystemMode.toUpperCase().contains("COOL")){ maxSetPoint=
	 * setPoints.get("MaxCool"); } if(maxSetPoint.contains(".")) {
	 * systemIsCelsius=true; maxSetPointFloat=Float.parseFloat(maxSetPoint); } else
	 * { maxSetPointInt=(int)Float.parseFloat(maxSetPoint); } WebElement
	 * ele=MobileUtils.getMobElement(objectDefinition,
	 * testCase,"UserExpectedTemperature"); currentSetPoint = ele.getText();
	 * 
	 * if(systemIsCelsius==false) {
	 * if(maxSetPointInt==(Integer.parseInt(currentSetPoint))) { return true; } }
	 * else { if(maxSetPointFloat==(Float.parseFloat(currentSetPoint))) { return
	 * true; } } return false; } /* public boolean
	 * isMinTemperatureVisibleOnDashBoard(TestCaseInputs inputs) { String
	 * currentSetPoint=""; // boolean flag = true; int minSetPointInt=0; float
	 * minSetPointFloat=0; String minSetPoint=""; boolean systemIsCelsius=false;
	 * DeviceInformation statInfo = new DeviceInformation(testCase, inputs); String
	 * currentSystemMode=statInfo.getThermoStatMode(); HashMap<String, String>
	 * setPoints = new HashMap<String, String>(); try {
	 * setPoints=statInfo.getDeviceMaxMinSetPoints(); } catch (Exception e) {
	 * e.printStackTrace(); } if(currentSystemMode.toUpperCase().contains("HEAT")) {
	 * minSetPoint= setPoints.get("MinHeat"); } else
	 * if(currentSystemMode.toUpperCase().contains("COOL")){ minSetPoint=
	 * setPoints.get("MinCool"); } if(minSetPoint.contains(".")) {
	 * systemIsCelsius=true; minSetPointFloat=Float.parseFloat(minSetPoint); } else
	 * { minSetPointInt=(int)Float.parseFloat(minSetPoint); } WebElement
	 * ele=MobileUtils.getMobElement(objectDefinition,
	 * testCase,"UserExpectedTemperature"); currentSetPoint = ele.getText();
	 * 
	 * if(systemIsCelsius==false) {
	 * if(minSetPointInt==(Integer.parseInt(currentSetPoint))) { return true; } }
	 * else { if(minSetPointFloat==(Float.parseFloat(currentSetPoint))) { return
	 * true; } } return false; }
	 * 
	 * 
	 * /* //Author : Vinayak A H public boolean selectDeviceFromDashboard(TestCases
	 * testCase, String locationToBeSelected, String deviceToBeSelected) { boolean
	 * flag = true; try { if
	 * (SuiteConstants.getConstantValue(SuiteConstantTypes.PROJECT_SPECIFIC,
	 * "TYPE_OF_TESTING").toUpperCase().equals("SIT")) {
	 * 
	 * 
	 * if(selectLocationFromDashBoard_NonStatic(testCase,locationToBeSelected)) {
	 * if(isDevicePresentOnDashboard(deviceToBeSelected)) { if
	 * (testCase.getPlatform().toUpperCase().contains("IOS")) { //TODO } else {
	 * MobileUtils.clickOnElement(testCase, "xpath",
	 * "//*[@text='"+deviceToBeSelected.trim()+"']"); } } else {
	 * MobileUtils.scrollTo(testCase, deviceToBeSelected).click(); }
	 * 
	 * LyricUtils.closeCoachMarks(testCase); } else flag = false; } else {
	 * System.out.
	 * println("selectDeviceFromDashboard : currently not apllicable for App PAT tests."
	 * ); }
	 * 
	...
	public boolean isDeviceOffline(int timeOut) {
		return MobileUtils.isMobElementExists(objectDefinition, testCase, "DeviceOffline", timeOut);
	}

	public boolean isActivityHistoryButtonVisible() {
		return MobileUtils.isMobElementExists(objectDefinition, testCase, "ActivityHistoryButton");
	}

	public boolean clickOnActivityHistory() {
		return MobileUtils.clickOnElement(objectDefinition, testCase, "ActivityHistoryButton");
	}

	public boolean isExpectedDeviceVisibleinDashboardAfterDelete() {
		if ((testCase.getPlatform().toUpperCase().contains("ANDROID") && MobileUtils.isMobElementExists("XPATH",
				"//*[@text='" + SystemState.getStatus().getTstatName() + "']", testCase))
				|| (testCase.getPlatform().toUpperCase().contains("IOS") && MobileUtils.isMobElementExists("XPATH",
						"//*[@name='" + SystemState.getStatus().getTstatName() + "']", testCase))) {
			return false;
		} else {
			return true;
		}
	...
	public boolean clickOndevice(String deviceName) {
		if(testCase.getPlatform().toUpperCase().contains("ANDROID")){
			return MobileUtils.clickOnElement(testCase, "XPATH", "//*[@text='"+deviceName+"']");
		}else {
	...
	public boolean isAnyDataAvailableInDashboard(String deviceName) {
		if(testCase.getPlatform().toUpperCase().contains("ANDROID")) {
			return MobileUtils.isMobElementExists("XPATH", "//*[@text='"+deviceName+"']/parent::android.widget.LinearLayout/following-sibling::android.widget.RelativeLayout//android.widget.ProgressBar", testCase);
		}else {
			return MobileUtils.isMobElementExists("XPATH", "", testCase);
		}
		
	}
	
	public boolean isDashboardScreen() {
		return MobileUtils.isMobElementExists("XPATH", "//android.widget.ImageButton[@content-desc='Main Menu']", testCase,10);
	}
