...
public class SchedulingScreen extends MobileScreens {
	...
	public boolean isYesButtonVisible(int timeOut) {
	...
	public boolean isSetPointChooserVisible(int timeOutInSec) 
	{
		return MobileUtils.isMobElementExists(objectDefinition, testCase, "CoolSetPointChooser", timeOutInSec);
	}
	...
	public boolean isScheduleScreenTitleVisible(int timeOut) 
	{
		if(MobileUtils.isMobElementExists(objectDefinition, testCase, "ToolbarTitle", timeOut)) 
		{
			if(testCase.getPlatform().toUpperCase().contains("ANDROID")) 
			{
				if(MobileUtils.getMobElement(objectDefinition, testCase, "ToolbarTitle").getText().toUpperCase().contains("SCHEDULE"))
					return true;
				else
					return false;
			}
	...
	public boolean clickOnFirstPeriod() 
	{
		if(MobileUtils.isMobElementExists(objectDefinition, testCase, "PeriodTitles", 5)) 
		{
			MobileUtils.getMobElements(objectDefinition, testCase, "PeriodTitles").get(0).click();
			return true;
		}
		else
			return false;
	...
	public boolean isGeofencingOff() 
	{
		if(testCase.getPlatform().toUpperCase().contains("ANDROID")) 
		{
				return false;
		...
		{
			if(MobileUtils.isMobElementExists(objectDefinition, testCase, "GeofencingOff", 10))
				return true;
			else
				return false;
		}
	...
	public boolean isUpdateLocPermissionVisible() 
	{
		if(testCase.getPlatform().toUpperCase().contains("IOS")) 
		{
			if(MobileUtils.isMobElementExists(objectDefinition, testCase, "UpdateLocPermission", 10))
				return true;
			else
				return false;	
		}
	...
	public boolean clickOnGoToSettings() 
	{
		if(MobileUtils.isMobElementExists(objectDefinition, testCase, "GoToSettings", 10)) 
		{
			return MobileUtils.clickOnElement(objectDefinition, testCase, "GoToSettings", "Clicked on Go To Settings...");
		}			
		else 
		{
			WebElement ele = testCase.getMobileDriver().findElement(By.name("Go To Settings"));
			if(ele != null)
			{
				ele.click();
				Keyword.ReportStep_Pass(testCase, "Clicked on Go To Settings...");
				return true;
			}
			else
				return false;
	...
	public boolean verifyNavigatedToHoneywellApp(int timeOut) 
	{
		if(MobileUtils.isMobElementExists(objectDefinition, testCase, "Settings", timeOut)) 
		{
			if(MobileUtils.isMobElementExists(objectDefinition, testCase, "HoneywellTitle", timeOut))
				return true;
			else
				return false;
		}		
	...
	public boolean ChangeLocAccessForHoneywellApp() 
	{
		boolean flag = false;
		if(MobileUtils.isMobElementExists(objectDefinition, testCase, "LocationPermission", 5)) 
		{
			flag = MobileUtils.clickOnElement(objectDefinition, testCase, "LocationPermission", "Clicked on Location button...");
			
			flag = flag && MobileUtils.clickOnElement(objectDefinition, testCase, "AlwaysPermission", "Clicked on Always...");
			
			flag = flag && MobileUtils.clickOnElement(objectDefinition, testCase, "ReturnToHoneywell", "Returning to Honeywell App...");			
		}
	...
	public boolean updateLocPermissionForApp() 
	{
		if(isUpdateLocPermissionVisible()) 
		{
			if(clickOnGoToSettings()) 
			{
				if(verifyNavigatedToHoneywellApp(20)) 
				{
					if(ChangeLocAccessForHoneywellApp()) 
					{
						Keyword.ReportStep_Pass(testCase, "Location Permission is changed to Always.");
						return true;						
					}
					else 
					{
						Keyword.ReportStep_Fail(testCase, FailType.FUNCTIONAL_FAILURE, "failed to change location permission");
						return false;						
					}
				}
				else 
				{
					Keyword.ReportStep_Fail(testCase, FailType.FUNCTIONAL_FAILURE, "failed to navigate to Honeywell app");
					return false;					
				}
			}
			else 
			{
				Keyword.ReportStep_Fail(testCase, FailType.FUNCTIONAL_FAILURE, "failed to click on Go To Settings or Go To Settings label not found");
				return false;					
			}
		}
		else 
		{
			Keyword.ReportStep_Fail(testCase, FailType.FUNCTIONAL_FAILURE, "Update Location Permission lable is not found");
			return false;					
		}
	}
}
