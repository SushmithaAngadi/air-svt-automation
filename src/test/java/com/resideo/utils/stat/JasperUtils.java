...
import com.resideo.utils.stat.cattgrpccommands.ISUMessage;
import com.resideo.utils.stat.cattgrpccommands.ISUMessage.ISUDevice;
...
//						.jasperSetISU(JasperIsuMsg.newBuilder().setIsu(IsuItem.valueOf(isuType.toUpperCase()))
...
////						Builder setEnumIsuVal = JasperIsuMsg.newBuilder().setIsu(IsuItem.valueOf(isuType));	
////						Builder setEnumIsuVal2 = JasperIsuMsg.newBuilder().setEnumIsuVal(IsuValue.ISU_200_HEAT_SYSTEM_TYPE_1__CONVENTIONAL);
////						System.out.println(setEnumIsuVal);
////						System.out.println(setEnumIsuVal2);
////						
////						StatCommands.getInstance().cattCommandsStub.jasperSetISU(JasperIsuMsg.newBuilder().setIsu(IsuItem.valueOf(isuType))
////						.setEnumIsuVal(IsuValue.ISU_200_HEAT_SYSTEM_TYPE_1__CONVENTIONAL).build());
//
//						isuValSet = true;
//						Keyword.ReportStep_Pass(testCase, "Stat Isu '" + isuType + "' is set to : '" + isuVal + "'");
//						break;
...
//	StatCommands.getInstance().cattCommandsStub.setISUToDevices(ISUMessage.newBuilder().setState(ISUDevice.JASPER_SR_NA_RETROFIT).setISUItem("ISU_HEAT_SYSTEM_TYPE_1").setISUValue("ISU_200_HEAT_SYSTEM_TYPE_1__HEAT_PUMP").build()).getBoolVal();
	...
	public static String verifyTempScale(TestCases testCase) {
		...
		ISUResponseMsg jasperGetISU = StatCommands.getInstance().cattCommandsStub
				.getISUToDevices(ISU_Settings.newBuilder().setISUType("ISU_TEMP_FORMAT").build());

		if (jasperGetISU.getByteIsuVal() == 0) {
	...
	public static boolean setJasper_Alert(TestCases testCase, String alertType, String alertValue) {
		boolean flag = false;
		try {
			flag = StatCommands.getInstance().cattCommandsStub.setAlertDevices(AlertmessageData.newBuilder()
					.setState(
							com.resideo.utils.stat.cattgrpccommands.AlertmessageData.ISUDevice.JASPER_SR_NA_RETROFIT)
	...
	public static boolean verifyJasper_Alert(TestCases testCase, String alertType, String alertValue) {
		boolean flag = false;
		try {
