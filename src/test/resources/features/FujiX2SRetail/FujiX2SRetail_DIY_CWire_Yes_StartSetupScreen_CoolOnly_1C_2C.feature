@FujiX2SRetail_DIY_CWire_Yes_StartSetupScreen_CoolOnly_1C_2C
Feature: FUJI Devices Registration on First Alert app using BLE flow
#   Switches needs to be On : None (Y, G are considered for C Wire) --- Y2 (Y, G are considered for C Wire)


	@FujiX2SRetail_DIY_CWire_Yes_FromStartUpScreen_CoolOnly_1C
	Scenario: FUJI DIY registration using FA app from Startup Screen state_CoolOnly_1C
	...
	When user verifies thermostat configuration as "None" with "0" heat and "1" cool stages in FA App
	Then Delete registered device from First alert APP
	
	
	@FujiX2SRetail_DIY_CWire_Yes_FromStartUpScreen_CoolOnly_2C
	Scenario: FUJI DIY registration using FA app from Startup Screen state_CoolOnly_2C
	And user broadcast FUJI device for registration from "START SETUP" screen 
	Given user launches and logs in to the "first alert" app
	Then Add new device to Configure in First Alert App
	Then Setup Device "Location and name" for DIY
	...
	Then First Alert app wifi "ALWAYS" configure the wifi with "CORRECT Password", is device factory reset "YES"
	Then user verifies for data synch after registration on First Alert APP
	When user verifies thermostat configuration as "None" with "0" heat and "2" cool stages in FA App
	Then Delete registered device from First alert APP
