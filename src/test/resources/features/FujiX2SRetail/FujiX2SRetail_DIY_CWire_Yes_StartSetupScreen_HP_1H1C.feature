@FujiX2SRetail_DIY_CWire_Yes_StartSetupScreen_HP_1H1C
Feature: FUJI Devices Registration on First Alert app using BLE flow
#   Switches needs to be On : O/B (Y, G are considered for C Wire)

	@FujiX2SRetail_DIY_CWire_Yes_FromStartUpScreen_HP_WarmAir_ElectricHeat_1H1C
	Scenario: FUJI DIY registration using FA app from Startup Screen state_HP_WarmAir_ElectricHeat_1H1C
	...
	@FujiX2SRetail_DIY_CWire_Yes_FromStartUpScreen_HP_CoolAir_ElectricHeat_1H1C
	Scenario: FUJI DIY registration using FA app from Startup Screen state_HP_CoolAir_ElectricHeat_1H1C
	And user broadcast FUJI device for registration from "START SETUP" screen 
	Given user launches and logs in to the "first alert" app
	Then Add new device to Configure in First Alert App
	Then Setup Device "Location and name" for DIY
