...
@HTA_HKRP01_Add_Thor_Thermostat_in_HomeKit_ISU_configured_with_registration_Station_Mode_DataSync_HTA_Registered 
@--xrayid:HTA-1632 
...
@HTA_HKRP02_Add_Thor_Thermostat_in_HomeKit_ISU_configured_with_registration_AP_Mode_DataSync_HTA_Registered 
@--xrayid:HTA-1427 
Scenario: HTA_1427 T10 plus HomeKit Paring with ISU configured with Registration AP mode with Data Sync 
...
@HTA_HKRP03_Add_Thor_Thermostat_in_HomeKit_ISU_configured_without_registration_AP_Mode_DataSync_HTA_Registered_No_Sensor 
@--xrayid:HTA-1427 
Scenario: HTA_1427 T10 plus HomeKit Paring with ISU configured without Registration Station mode with Data Sync no sensor 

	Given user thermostat should be "register" in cloud 
	Given user launches Home Kit application 
	Then Check stat is paired with HomeKit 
	Then Initiate HomeKit in "Station Mode" Thermostat from Add Accessory 
	Then Pair Thermostat using "QR Scan" in Homekit 
	...
	Then Set "Heat" Setpoint "69" from "HOMEKIT" ,Verify from "STAT" 
	Then Delete Device from HomeKit 
	Given user thermostat should be "unregister" in cloud 
	And check thermostat is unregister"true" in cloud, if not delete from cloud using cloud wrapper API 
	Then verify stat is unregistered from cloud 
