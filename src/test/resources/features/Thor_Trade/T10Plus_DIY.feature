@Thor_DIY_realDevice_CHECKUI
Feature: Thor Registration 
	Register the Thor device using user DIY flow 

	
@Thor_Diy_RealDevice_Conventional_2C2H_GUI_install_HappyPath_UIRESET
...
#	When User verify the thermostat having Wifi configuration, if not configure WIFI through AAT
	Then User open QR Code from Connect app screen
...
#	Then User select GUI ISU Type "Floor Sensor" and ISU Name as "Wired" 
#	Then User select GUI ISU Type "Outdoor Temperature" and ISU Name as "Wireless" 
#	Then User select GUI ISU Type "Equipment Type" and ISU Name as "Hot Water Radiant Floor Heat" 
